...
function pageLockHtml(pageKey, untilMs) {
  const label = (LOCKABLE_PAGES[pageKey] && LOCKABLE_PAGES[pageKey].label) || pageKey;
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>
  *{box-sizing:border-box}
  ...
  .back-btn{
    position:fixed;top:16px;left:16px;background:transparent;border:1px solid rgba(255,255,255,.15);
    color:rgba(255,255,255,.6);font-size:.72rem;font-weight:600;padding:7px 14px 7px 10px;border-radius:20px;
    cursor:pointer;font-family:inherit;transition:all .2s ease;display:flex;align-items:center;gap:5px;
  }
...
export async function pageLockGate(req, res, next) {
  let activeLocks;
  try {
    activeLocks = await getPageLockStatus();
  } catch {
    return next();
  }
  if (!activeLocks || !Object.keys(activeLocks).length) return next();

  const lockedKey = findLockedPageForPath(activeLocks, req.path);
  if (!lockedKey) return next();
  ...
  const entry = activeLocks[lockedKey];
  res.set("Cache-Control", "no-store");
  res.set("Retry-After", "3600");
  if (req.path.startsWith("/api/") || req.path.startsWith("/embed/")) {
    return res.status(503).json({
      error: `${(LOCKABLE_PAGES[lockedKey] && LOCKABLE_PAGES[lockedKey].label) || lockedKey} is temporarily locked. Please try again later.`,
      lockedUntil: entry.until || null,
    });
  }
  return res.status(503).type("html").send(pageLockHtml(lockedKey, entry.until));
}
