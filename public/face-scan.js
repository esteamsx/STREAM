...
function warmUpModels() {
  ...
  warmupPromise = (async () => {
    ...
    try {
      ...
      await faceapi.nets.faceRecognitionNet.computeFaceDescriptor(warm);
    } catch (e) {}
    return faceapi;
  })();
...
const DIAG_KEY = 'esFaceDiag';

function diagEnabled() {
  try {
    const q = new URLSearchParams(window.location.search).get('facediag');
    if (q === '1') {
      window.localStorage.setItem(DIAG_KEY, '1');
      return true;
    }
    if (q === '0') {
...
#${OVERLAY_ID}.fs-top-mode .fs-abstract,#${OVERLAY_ID}.fs-top-mode .fs-status,#${OVERLAY_ID}.fs-top-mode .fs-cancel{
  pointer-events:auto}
#${OVERLAY_ID}.fs-top-mode .fs-abstract{width:112px;height:112px}
#${OVERLAY_ID}.fs-top-mode .fs-abstract-face{width:46px;height:46px}
#${OVERLAY_ID}.fs-top-mode .fs-abstract-check,#${OVERLAY_ID}.fs-top-mode .fs-abstract-cross{width:42px;height:42px}
...
export function captureFaceDescriptor({ requireLiveness = true, showCamera = true, verify = null, voice = true, samples = DEFAULT_SAMPLES, returnSamples = false } = {}) {
  ...
  return new Promise((resolve, reject) => {
    watchdogTimer = setTimeout(() => {
      if (cancelled) return;
      cancelled = true;
      console.log('[face-scan] startup watchdog fired before scanning began');
      cleanup();
    ...
    (async () => {
      try {
        ...
        function scanCode(){
          ...
          return ' [f' + noFaceCount + ' s' + tooSmallCount + ' w' + slowFrames +
            ' e' + detectErrors + ' ' + backend + ' ' + srcW + 'x' + srcH + ' ' +
        ...
        watchdogTimer = setTimeout(() => {
          if (cancelled) return;
          console.log('[face-scan] watchdog fired: ' + scanReport());
          cleanup();
          reject(new Error('Face check stalled, please try again.' + scanCode()));
        }, CAPTURE_TIMEOUT_MS + WATCHDOG_GRACE_MS);
        ...
        const runDetect = async (withLandmarks, withDescriptor) => {
          ...
          try {
            ...
            if (Date.now() - started > SLOW_FRAME_MS) slowFrames++;
            return out;
          } catch (err) {
            detectErrors++;
        ...
        const giveUp = (label) => {
          console.log('[face-scan] ' + label + ': ' + scanReport());
          cleanup();
          let message;
          if (detectErrors > noFaceCount) {
            message = 'The face engine failed on this device. Use your password to sign in.';
          } else if (slowFrames > 0 && attemptCount() < MIN_NO_FACE_ATTEMPTS) {
            message = 'This device is too slow to run the face check right now. Try again or use your password.';
          } else if (tooSmallCount > noFaceCount) {
            message = 'Hold the phone a bit closer to your face and try again.';
          } else {
            message = "Couldn't see your face. Make sure the camera isn't blocked, move somewhere brighter, and try again.";
          }
          reject(new Error(message + scanCode()));
        };
        ...
        const tick = async () => {
          ...
          if (!requireLiveness && collected.length === 0 && elapsed > NO_FACE_TIMEOUT_MS &&
              attemptCount() >= MIN_NO_FACE_ATTEMPTS) {
          ...
          if (cancelled) return;
          if (probe === DETECT_FAILED) {
            again(safeTick, ERROR_BACKOFF_MS);
            return;
          }
          if (!probe) {
            noFaceCount++;
