...
const passwordLimiter = new SimpleRateLimiter(30, 60 * 1000, (req) => req.apiKeyId || req.ip).middleware();
const hashLimiter = new SimpleRateLimiter(30, 60 * 1000, (req) => req.apiKeyId || req.ip).middleware();
...
const imagegenLimiter = new SimpleRateLimiter(10, 60 * 1000, (req) => req.apiKeyId || req.ip).middleware();
const bgremoveLimiter = new SimpleRateLimiter(10, 60 * 1000, (req) => req.apiKeyId || req.ip).middleware();
const wacheckLimiter = new SimpleRateLimiter(15, 60 * 1000, (req) => req.apiKeyId || req.ip).middleware();
...
router.get("/api/v1/dev/password", requireDevApiKey, passwordLimiter, async (req, res) => {
  try {
    const result = generatePassword(req.query.length, {
    ...
    res.status(err.status || 502).json({ error: err.message || "Could not generate a password." });
  }
});

router.post("/api/v1/dev/hash", requireDevApiKey, hashLimiter, async (req, res) => {
  const text = String(req.body?.text || "").trim();
  const mode = String(req.body?.mode || "hash").trim().toLowerCase();
  if (!text) return res.status(400).json({ error: "Missing text field." });

  try {
    let result;
    if (mode === "base64encode") result = encodeBase64(text);
    else if (mode === "base64decode") result = decodeBase64(text);
    else result = hashText(text, req.body?.algorithm);
    res.json(result);
  } catch (err) {
    res.status(err.status || 502).json({ error: err.message || "Could not process that request." });
  }
});

router.get("/api/v1/dev/screenshot", requireDevApiKey, screenshotLimiter, async (req, res) => {
  const url = String(req.query.url || "").trim();
  if (!url) return res.status(400).json({ error: "Missing url query parameter." });
...
router.get("/api/v1/dev/translate", requireDevApiKey, translateLimiter, async (req, res) => {
  const text = String(req.query.text || "").trim();
  const to = String(req.query.to || "").trim();
  if (!text) return res.status(400).json({ error: "Missing text query parameter." });
  if (!to) return res.status(400).json({ error: "Missing to query parameter (target language code, e.g. es, fr, ja)." });
  if (text.length > 500) return res.status(400).json({ error: "Text is too long (500 character limit)." });

  try {
    const result = await translateText(text, req.query.from, to);
    res.json(result);
  } catch (err) {
    res.status(err.status || 502).json({ error: err.message || "Could not translate that text." });
  }
});

router.post("/api/v1/dev/summarize", requireDevApiKey, summarizeLimiter, async (req, res) => {
  const text = String((req.body && req.body.text) || "").trim();
  if (!text) return res.status(400).json({ error: "Missing text in request body." });
  if (text.length > 6000) return res.status(400).json({ error: "Text is too long (6000 character limit)." });

  try {
    const prompt = `Summarize the following text in 2-4 concise sentences, output only the summary with no preamble:\n\n${text}`;
    const summary = await askFreeAI(prompt);
...
router.get("/api/v1/dev/imagegen", requireDevApiKey, imagegenLimiter, async (req, res) => {
  ...
  if (prompt.length > 500) return res.status(400).json({ error: "Prompt is too long (500 character limit)." });
  try {
    const { buffer, contentType } = await generateImageFromPrompt(prompt, {
    ...
    res.status(err.status || 502).json({ error: err.message || "Could not generate that image." });
  }
});

router.post("/api/v1/dev/bg-remove", requireDevApiKey, bgremoveLimiter, async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: "image_url must be a valid public http(s) link." });
  }
  try {
    const result = await removeImageBackground(imageUrl);
    res.json({ image_url: result.imageUrl });
  } catch (err) {
    res.status(err.status || 502).json({ error: err.message || "Could not remove the background." });
  }
});

router.get("/api/v1/dev/wa-check", requireDevApiKey, wacheckLimiter, async (req, res) => {
  const number = String(req.query.number || "").trim();
  if (!number) return res.status(400).json({ error: "Missing number query parameter (digits only, with country code)." });
  try {
    const result = await checkWhatsAppNumberViaBotService(number);
    res.json(result);
  } catch (err) {
    res.status(err.status || 502).json({ error: err.message || "Could not check that number." });
  }
});
