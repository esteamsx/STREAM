...
<script nonce="__CSP_NONCE__" src="/push-client.js" defer></script>
...
.sc-bubble{position:relative;max-width:78%;padding:8px 12px;border-radius:14px;font-size:.84rem;line-height:1.4;white-space:pre-wrap;word-break:break-word;box-shadow:0 1px 2px rgba(0,0,0,.2);user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}
...
.sc-msg-row-inner{display:flex;flex-direction:column;transition:transform .15s var(--ease)}
.sc-msg-row.sc-row-me .sc-msg-row-inner{align-items:flex-end}
.sc-msg-row.sc-row-them .sc-msg-row-inner{align-items:flex-start}
.sc-reply-icon{
  position:absolute;top:50%;left:8px;transform:translateY(-50%) scale(.6);width:26px;height:26px;border-radius:50%;
  background:var(--card2);border:1px solid var(--border-strong);color:var(--accent);display:flex;align-items:center;
  justify-content:center;opacity:0;transition:opacity .15s var(--ease),transform .15s var(--ease);pointer-events:none;
}
.sc-reply-icon svg{width:14px;height:14px}
.sc-reply-icon.show{opacity:1;transform:translateY(-50%) scale(1)}
.sc-reply-quote{
  display:block;padding:6px 8px;margin-bottom:5px;border-left:3px solid currentColor;border-radius:6px;
  background:rgba(0,0,0,.12);font-size:.76rem;opacity:.85;cursor:pointer;
}
.sc-reply-quote-label{font-weight:700;display:block;margin-bottom:1px}
.sc-reply-quote-text{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:220px}
.sc-msg-menu-overlay{align-items:flex-end}
.sc-msg-menu{
  width:100%;background:var(--card);border-radius:20px 20px 0 0;padding:8px 0 max(8px,env(safe-area-inset-bottom));
  box-shadow:0 -8px 30px rgba(0,0,0,.4);
}
.sc-msg-menu-btn{
  display:flex;align-items:center;gap:12px;width:100%;padding:14px 20px;background:transparent;border:none;
  color:var(--text);font-size:.9rem;font-weight:600;text-align:left;
}
.sc-msg-menu-btn svg{width:19px;height:19px;flex-shrink:0}
.sc-msg-menu-btn:disabled{opacity:.35}
.sc-msg-menu-danger{color:#ff3b5c}
.sc-composer-context{
...
.sc-composer-context-inner{flex:1;min-width:0;border-left:3px solid var(--accent);padding-left:10px}
.sc-composer-context-label{font-size:.72rem;font-weight:700;color:var(--accent)}
.sc-composer-context-text{font-size:.78rem;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sc-composer-context-cancel{flex-shrink:0;width:26px;height:26px;border-radius:50%;background:transparent;border:none;color:var(--muted);display:flex;align-items:center;justify-content:center}
.sc-composer-context-cancel svg{width:16px;height:16px}
.sc-chat-footer{display:flex;gap:8px;padding:12px 14px;border-top:1px solid var(--border);flex-shrink:0}
.sc-jump-highlight .sc-bubble{outline:2px solid var(--accent);outline-offset:1px}
.sc-chat-input{
...
.sc-pv-ico{position:relative;display:inline-block;width:12px;height:12px;margin-right:6px;vertical-align:-1px;
  color:var(--muted);flex-shrink:0}
.sc-pv-image{border:1.4px solid currentColor;border-radius:2.5px}
.sc-pv-image::before{content:'';position:absolute;left:1.5px;bottom:1.5px;width:0;height:0;
  border-left:3.5px solid transparent;border-right:3.5px solid transparent;border-bottom:4.5px solid currentColor}
.sc-pv-image::after{content:'';position:absolute;top:2px;right:2px;width:2.4px;height:2.4px;border-radius:50%;background:currentColor}
.sc-pv-file{border:1.4px solid currentColor;border-radius:2px;border-top-right-radius:0}
.sc-pv-file::after{content:'';position:absolute;top:-1.4px;right:-1.4px;width:0;height:0;
  border-left:4.5px solid transparent;border-bottom:4.5px solid currentColor}
.sc-pv-voice::before{content:'';position:absolute;left:3.3px;top:0;width:5.4px;height:7.5px;border-radius:3px;background:currentColor}
.sc-pv-voice::after{content:'';position:absolute;left:1.5px;bottom:0;width:9px;height:4.5px;
  border:1.4px solid currentColor;border-top:none;border-bottom-left-radius:5px;border-bottom-right-radius:5px}
...
.rw-send-amount-row{display:flex;gap:8px;align-items:flex-end}
.rw-send-amount-row .field{flex:1;margin-bottom:0}
.rw-send-btn{display:flex;align-items:center;justify-content:center;gap:7px;white-space:nowrap;flex-shrink:0}
.rw-send-btn svg{width:15px;height:15px;flex-shrink:0}
.rw-send-limits{font-size:.72rem;color:var(--muted);margin-top:8px;line-height:1.55}
.rw-send-limits span{display:block}
.rw-send-limits b{color:var(--text);font-weight:700}
...
.rw-actions{display:flex;gap:10px;margin-bottom:18px}
.rw-action{
  flex:1;display:flex;flex-direction:column;align-items:center;gap:9px;padding:15px 6px;
  background:linear-gradient(155deg,rgba(255,255,255,.1),rgba(255,255,255,.02) 40%,rgba(255,255,255,.04) 100%),rgba(255,255,255,.045);
  border:1px solid rgba(255,255,255,.16);border-radius:16px;
  box-shadow:0 16px 40px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.12);
  color:var(--text);font-family:var(--font-body);font-size:.74rem;font-weight:600;
  cursor:pointer;transition:transform .16s ease,border-color .16s ease;
}
:root[data-theme="light"] .rw-action{
  background:linear-gradient(155deg,rgba(255,255,255,.5),rgba(255,255,255,.16) 40%,rgba(255,255,255,.24) 100%);
  border:1px solid rgba(255,255,255,.55);
  box-shadow:0 16px 40px rgba(20,20,28,.1),inset 0 1px 0 rgba(255,255,255,.65);
}
.rw-action:active{transform:scale(.96)}
.rw-action:hover{border-color:rgba(0,224,255,.4)}
.rw-action-ico{
  width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;
  background:linear-gradient(140deg,rgba(0,224,255,.2),rgba(124,92,255,.2));
  border:1px solid rgba(0,224,255,.3);color:var(--cyan);
}
:root[data-theme="light"] .rw-action-ico{background:linear-gradient(140deg,rgba(0,224,255,.22),rgba(124,92,255,.2));border-color:rgba(0,168,198,.4);color:#0083a0}
.rw-action-ico svg{width:19px;height:19px}
.rw-action-label{letter-spacing:.01em}
.overlay-card.rw-tall{max-height:86vh;overflow-y:auto;scrollbar-width:thin}
.overlay-card.rw-tall > .field{margin-bottom:0}
.overlay-card.rw-tall > .rw-send-limits{margin-top:0}
.overlay-card.rw-tall::-webkit-scrollbar{width:5px}
.overlay-card.rw-tall::-webkit-scrollbar-thumb{background:rgba(255,255,255,.18);border-radius:3px}
:root[data-theme="light"] .overlay-card.rw-tall::-webkit-scrollbar-thumb{background:rgba(20,20,28,.22)}
.rw-receive-rate{font-size:.78rem;color:var(--muted);line-height:1.6;min-height:1.4em}
.rw-receive-rate b{color:var(--text);font-weight:700}
.rw-once-warn{
  display:flex;gap:9px;align-items:flex-start;padding:11px 13px;border-radius:12px;
  background:rgba(255,183,51,.1);border:1px solid rgba(255,183,51,.3);
  font-size:.76rem;line-height:1.55;color:var(--text);
}
.rw-once-warn svg{width:15px;height:15px;flex-shrink:0;margin-top:2px;color:#F4B733}
.rw-receive-summary{font-size:.74rem;color:var(--muted);margin-top:10px;line-height:1.6}
.rw-receive-summary b{color:var(--text);font-weight:700}
...
:root[data-theme="light"] .bank-picker-list{
...
.rw-bank-card-brand{position:relative;display:flex;align-items:center;gap:11px;margin-bottom:26px}
.rw-bank-card-logo{
  position:relative;width:42px;height:42px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;
  ...
  box-shadow:0 3px 10px rgba(0,0,0,.35),inset 0 0 0 1px rgba(0,0,0,.06);
}
.rw-bank-card-logo::before{
  content:'';position:absolute;inset:0;border-radius:50%;
  background:linear-gradient(135deg,rgba(255,255,255,.9),transparent 40%,rgba(0,0,0,.03));pointer-events:none;
}
.rw-bank-card-logo::after{
  content:'';position:absolute;inset:0;border-radius:50%;
  box-shadow:inset 0 0 0 2px var(--bm-fg,#5a4fcf);pointer-events:none;opacity:.9;
}
.rw-bank-card-logo.wide{font-size:.56rem;letter-spacing:.01em}
.rw-bank-card-logo-mono{position:relative;z-index:1;color:var(--bm-fg,#5a4fcf)}
.rw-bank-card-brand-text{
  font-family:var(--font-display);font-weight:800;font-size:.86rem;letter-spacing:.06em;text-transform:uppercase;
  color:#fff;text-shadow:0 1px 3px rgba(0,0,0,.35);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;
}
.rw-bank-card-network{position:relative;width:52px;height:30px;flex-shrink:0}
.rw-bank-card-network-c1,.rw-bank-card-network-c2{
  position:absolute;top:0;width:30px;height:30px;border-radius:50%;
  box-shadow:0 2px 6px rgba(0,0,0,.3);
}
.rw-bank-card-network-c1{left:0;background:linear-gradient(135deg,#00E0FF,#00A8C6)}
.rw-bank-card-network-c2{right:0;background:linear-gradient(135deg,#8A6BFF,#5a4fcf);opacity:.92}
...
.bm-zenith{
  ...
  background:linear-gradient(180deg,#58595B 0%,#58595B 50%,#E4002B 50%,#E4002B 100%);
  clip-path:polygon(6% 6%,94% 6%,94% 24%,38% 76%,94% 76%,94% 94%,6% 94%,6% 76%,62% 24%,6% 24%);
}
.bm-access-core{
  position:absolute;inset:0;margin:auto;width:9px;height:9px;background:var(--bm-fg,#F26522);
  transform:rotate(45deg);z-index:2;
}
.bm-access-r1{position:absolute;inset:11px;border:2px solid var(--bm-fg,#F26522);transform:rotate(45deg);z-index:1}
.bm-access-r2{position:absolute;inset:6px;border:2px solid var(--bm-fg,#F26522);opacity:.5;transform:rotate(45deg);z-index:1}
...
<div class="acc-wrap" id="accLoader" aria-hidden="true">
  <div class="acc-hero">
    <div class="sk-avatar-lg"></div>
    <div class="sk-hero-body">
      <div class="sk-line" style="width:52%;height:16px"></div>
      <div class="sk-line" style="width:34%;height:11px"></div>
    </div>
  </div>

  <div class="acc-card">
    <div class="sk-line" style="width:32%;height:13px;margin-bottom:18px"></div>
    <div class="sk-field-row">
      <div class="sk-field"><div class="sk-line" style="width:56%;height:9px"></div><div class="sk-input"></div></div>
      <div class="sk-field"><div class="sk-line" style="width:50%;height:9px"></div><div class="sk-input"></div></div>
    </div>
    <div class="sk-field"><div class="sk-line" style="width:26%;height:9px"></div><div class="sk-input"></div></div>
    <div class="sk-field"><div class="sk-line" style="width:34%;height:9px"></div><div class="sk-input"></div></div>
  </div>

  <div class="acc-card">
    <div class="sk-line" style="width:26%;height:13px;margin-bottom:18px"></div>
    <div class="sk-field"><div class="sk-line" style="width:38%;height:9px"></div><div class="sk-input"></div></div>
    <div class="sk-btn full" style="margin-top:16px"></div>
  </div>

  <div class="acc-card">
    <div class="sk-line" style="width:36%;height:13px;margin-bottom:18px"></div>
    <div class="sk-stack">
      <div class="sk-inline"><div class="sk-avatar" style="width:34px;height:34px;border-radius:10px"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div>
      <div class="sk-inline"><div class="sk-avatar" style="width:34px;height:34px;border-radius:10px"></div><div class="sk-row-body"><div class="sk-line w45"></div><div class="sk-line w30"></div></div></div>
    </div>
  </div>
</div>
...
<div class="acc-wrap" id="accWrap" style="display:none">
  ...
  <div class="tfa-card" id="billingCard">
    <div class="tfa-header" id="billingHeader">
      <svg class="tfa-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M2 10h20"/><path stroke-linecap="round" d="M6 14.5h3"/></svg>
      <div class="tfa-header-title">Billing &amp; Auto-Renew</div>
      <svg class="tfa-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/></svg>
    </div>
    <div class="tfa-body">
      <div class="tfa-body-inner">
        <div style="font-size:.78rem;color:var(--muted);margin-bottom:12px;line-height:1.5">Keep a subscription running without paying again by hand. If a renewal charge fails, the subscription simply ends on its expiry date.</div>

        <div id="billingCardRow" style="display:none">
          <div class="pk-row">
            <div>
              <div class="pk-row-name" id="billingCardName">Card</div>
              <div class="pk-row-time" id="billingCardMeta"></div>
            </div>
            <button class="pk-delete-btn" id="billingRemoveCardBtn">
        ...
        <div id="billingNoCardRow" style="font-size:.78rem;color:var(--muted);line-height:1.5;padding:10px 0">No card saved yet. Pay for verification or an API plan once and the card is saved here for renewals.</div>

        <div id="billingToggles" style="margin-top:14px"></div>
        <div class="acc-msg" id="billingMsg"></div>
      </div>
    </div>
  </div>
  ...
  <div id="view-rewards" class="acc-view-panel">
    ...
    <div class="rw-actions">
      <button type="button" class="rw-action" id="rwActionSend">
        <span class="rw-action-ico">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 8h13m0 0l-3.5-3.5M17 8l-3.5 3.5"/><path stroke-linecap="round" stroke-linejoin="round" d="M20 16H7m0 0l3.5-3.5M7 16l3.5 3.5"/></svg>
        </span>
        <span class="rw-action-label">Send</span>
      </button>
      <button type="button" class="rw-action" id="rwActionReceive">
        <span class="rw-action-ico">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v11m0 0l-4.5-4.5M12 15l4.5-4.5"/><path stroke-linecap="round" d="M4 19h16"/></svg>
        </span>
        <span class="rw-action-label">Receive</span>
      </button>
      <button type="button" class="rw-action" id="rwActionCards">
        <span class="rw-action-ico">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M2 10h20"/><path stroke-linecap="round" d="M6 14.5h3"/></svg>
        </span>
        <span class="rw-action-label">Cards</span>
      </button>
    </div>
...
<div class="page-overlay" id="rwSendConfirmOverlay">
  <div class="overlay-card" style="max-width:360px">
    <div class="overlay-title">Are you sure?</div>
    <div class="overlay-sub" id="rwSendConfirmSub"></div>
    <div class="rw-hint">Coin transfers are final. They cannot be reversed once sent, so check the username carefully.</div>
    <div style="margin:12px 0"><altcha-widget id="rwSendAltcha" challengeurl="/api/captcha/challenge" workers="4"></altcha-widget></div>
    <div class="acc-msg" id="rwSendConfirmMsg"></div>
    <button class="acc-btn rw-send-btn" id="rwSendConfirmBtn" type="button" style="width:100%" disabled>
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 8h13m0 0l-3.5-3.5M17 8l-3.5 3.5"/><path stroke-linecap="round" stroke-linejoin="round" d="M20 16H7m0 0l3.5-3.5M7 16l3.5 3.5"/></svg>
      <span>Transfer</span>
    </button>
    <button class="overlay-cancel" id="rwSendCancelBtn" type="button">Close</button>
  </div>
</div>

<div class="page-overlay" id="rwSendOverlay">
  <div class="overlay-card rw-tall">
    <div class="overlay-title">Send Coins</div>
    <div class="overlay-sub">Send coins to another ES TEAMS TV user with their username.</div>
    <div class="field">
      <label>Recipient username</label>
      <input type="text" id="rwSendUsername" placeholder="username" autocomplete="off" autocapitalize="none" spellcheck="false" maxlength="20">
      <div class="uname-status" id="rwSendUsernameStatus"></div>
    </div>
    <div class="rw-send-amount-row">
      <div class="field">
        <label>Amount (coins)</label>
        <input type="number" id="rwSendAmount" placeholder="5" min="5" max="100" step="1">
      </div>
      <button type="button" class="acc-btn rw-send-btn" id="rwSendBtn" disabled>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 8h13m0 0l-3.5-3.5M17 8l-3.5 3.5"/><path stroke-linecap="round" stroke-linejoin="round" d="M20 16H7m0 0l3.5-3.5M7 16l3.5 3.5"/></svg>
        <span>Transfer</span>
      </button>
    </div>
    <div class="rw-send-limits" id="rwSendLimits"><span>Minimum <b>5 coins</b>, maximum <b>100 coins</b> per transfer.</span></div>
    <div class="acc-msg" id="rwSendMsg"></div>
    <button class="overlay-cancel" id="rwSendOverlayCloseBtn" type="button">Close</button>
  </div>
</div>

<div class="page-overlay" id="rwReceiveOverlay">
  <div class="overlay-card rw-tall">
    <div class="overlay-title">Receive Coins</div>
    <div class="overlay-sub">Generate a payment link. Anyone can open it and pay, and the coins land straight in your balance.</div>
    <div id="rwReceiveForm">
      <div class="field">
        <label>Amount to receive (coins)</label>
        <input type="number" id="rwReceiveCoins" placeholder="20" min="20" max="5000" step="1">
      </div>
      <div class="rw-receive-rate" id="rwReceiveRate"></div>
      <div class="rw-hint">This is the pay-anyone rate, so it costs more per coin than buying from the Coin Store yourself.</div>
      <button type="button" class="acc-btn" id="rwReceiveGenBtn" style="width:100%;margin-top:12px">Generate Payment Link</button>
    </div>
    <div id="rwReceiveResult" style="display:none">
      <div class="rw-once-warn">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><path stroke-linecap="round" d="M12 9v4M12 17h.01"/></svg>
        <span>Copy this link now. It is shown one time only and expires in 1 hour.</span>
      </div>
      <div class="rw-ref-link-row" style="margin-top:12px">
        <input type="text" id="rwReceiveLink" readonly>
        <button type="button" class="acc-btn" id="rwReceiveCopyBtn">Copy</button>
      </div>
      <div class="rw-receive-summary" id="rwReceiveSummary"></div>
      <button type="button" class="acc-btn acc-btn-ghost" id="rwReceiveAgainBtn" style="width:100%;margin-top:12px">Generate Another</button>
    </div>
    <div class="acc-msg" id="rwReceiveMsg"></div>
    <button class="overlay-cancel" id="rwReceiveCloseBtn" type="button">Close</button>
  </div>
</div>

<div class="page-overlay" id="rwCardsOverlay">
  <div class="overlay-card rw-tall">
    <div class="overlay-title">Cards</div>
    <div class="overlay-sub">The bank account your withdrawals are paid into. One card per account.</div>
      ...
      <div class="rw-bank-card">
        ...
        <div class="rw-bank-card-bottom">
          <div>
            ...
            <span class="sr-only" id="rwBankCardBankName"></span>
          </div>
          <div class="rw-bank-card-network" aria-hidden="true">
            <span class="rw-bank-card-network-c1"></span>
            <span class="rw-bank-card-network-c2"></span>
          </div>
        </div>
      </div>
    </div>
    <button class="overlay-cancel" id="rwCardsCloseBtn" type="button">Close</button>
  </div>
</div>

<div class="page-overlay" id="rwClaimOverlay">
  <div class="overlay-card">
    <div class="verify-icon-wrap">
      <svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 7v10M8.5 9.5a2 2 0 013-1.7M15.5 14.5a2 2 0 01-3 1.7"/></svg>
    </div>
    <div class="overlay-title" id="rwClaimTitle" style="text-align:center">Claim Daily Coins</div>
    <div class="overlay-sub" id="rwClaimSub" style="text-align:center">Verify the captcha, then hold still for a quick face check.</div>

    <div id="rwClaimChecking">
      <div class="sk-line w60" style="margin:0 auto 10px"></div>
      <div class="sk-line w80" style="margin:0 auto 16px"></div>
    ...
    <div id="rwClaimNoFaceId" style="display:none">
      <div class="rw-hint">Your Face Id is what proves the claim is really you. Set it up once in your account settings, then you can claim every day.</div>
      <button class="acc-btn" id="rwClaimGoFaceIdBtn" type="button" style="width:100%;margin-top:12px">Go to Face Id</button>
    </div>

    <button class="overlay-cancel" id="rwClaimCloseBtn" type="button">Close</button>
  </div>
</div>
...
<div class="page-overlay" id="rwBankDeleteOverlay">
  <div class="overlay-card" style="max-width:320px;text-align:center">
    ...
    <div style="margin:12px 0;text-align:left"><altcha-widget id="rwBankDeleteAltcha" challengeurl="/api/captcha/challenge" workers="4"></altcha-widget></div>
...
<div class="page-overlay" id="notifOverlay">
  <div class="flist-card">
    ...
    <div class="flist-search-wrap" style="padding-top:0;display:flex;justify-content:flex-end;align-items:center;gap:8px">
      <button type="button" class="notif-mark-read" id="notifEnablePushBtn" style="display:none;margin-right:auto">Enable notifications</button>
...
<div class="page-overlay" id="supportChatOverlay">
  <div class="flist-card sc-chat-card">
    ...
    <div class="sc-composer-context" id="scComposerContext" style="display:none">
      <div class="sc-composer-context-inner">
        <div class="sc-composer-context-label" id="scComposerContextLabel"></div>
        <div class="sc-composer-context-text" id="scComposerContextText"></div>
      </div>
      <button type="button" class="sc-composer-context-cancel" id="scComposerContextCancel" aria-label="Cancel">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path stroke-linecap="round" d="M18 6L6 18M6 6l12 12"/></svg>
      </button>
    </div>
    <div class="sc-chat-footer">
      ...
      <textarea id="supportChatInput" class="sc-chat-input" maxlength="2000" rows="1" placeholder="Type a message…" autocomplete="off" enterkeyhint="enter"></textarea>
...
<div class="page-overlay sc-msg-menu-overlay" id="scMsgMenuOverlay">
  <div class="sc-msg-menu">
    <button type="button" class="sc-msg-menu-btn" id="scMsgEditBtn">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M17 3a2.85 2.83 0 114 4L7.5 20.5 2 22l1.5-5.5z"/></svg>
      <span>Edit</span>
    </button>
    <button type="button" class="sc-msg-menu-btn sc-msg-menu-danger" id="scMsgDeleteBtn">
    ...
    <button type="button" class="sc-msg-menu-btn" id="scMsgMenuCancelBtn">
      <span>Cancel</span>
    </button>
  </div>
</div>
...
document.getElementById('notifEnablePushBtn').addEventListener('click', async () => {
  const btn = document.getElementById('notifEnablePushBtn');
  btn.disabled = true;
...
document.getElementById('billingHeader').addEventListener('click', () => {
  document.getElementById('billingCard').classList.toggle('open');
});

let billingState = null;

function fmtRenewDate(ms){
  if (!ms) return '';
  return new Date(ms).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
}

function renderBilling(){
  const cardRow = document.getElementById('billingCardRow');
  const noCardRow = document.getElementById('billingNoCardRow');
  const togglesEl = document.getElementById('billingToggles');
  const card = billingState && billingState.card;

  if (card) {
    cardRow.style.display = '';
    noCardRow.style.display = 'none';
    const brand = (card.brand || 'Card').replace(/\b\w/g, (c) => c.toUpperCase());
    document.getElementById('billingCardName').textContent = brand + ' •••• ' + (card.last4 || '____');
    const bits = [];
    if (card.expMonth && card.expYear) bits.push('Expires ' + card.expMonth + '/' + String(card.expYear).slice(-2));
    if (card.bank) bits.push(card.bank);
    document.getElementById('billingCardMeta').textContent = bits.join(' · ');
  } else {
    cardRow.style.display = 'none';
    noCardRow.style.display = '';
  }

  const subs = (billingState && billingState.subscriptions) || {};
  const keys = Object.keys(subs);
  togglesEl.innerHTML = keys.map((key) => {
    const s = subs[key];
    let sub;
    if (s.autoRenew) sub = s.expiresAt ? ('Renews ' + fmtRenewDate(s.expiresAt)) : 'On';
    else if (s.active) sub = 'Ends ' + fmtRenewDate(s.expiresAt);
    else sub = 'Not active';
    return '<div class="tfa-toggle-row" style="margin-bottom:14px">' +
      '<div><div class="tfa-toggle-label">' + esc(s.label) + '</div>' +
      '<div class="tfa-toggle-sub">' + esc(sub) + '</div></div>' +
      '<div class="tfa-switch' + (s.autoRenew ? ' on' : '') + '" data-renew="' + esc(key) + '"><div class="tfa-switch-dot"></div></div>' +
      '</div>';
  }).join('');

  togglesEl.querySelectorAll('[data-renew]').forEach((sw) => {
    sw.addEventListener('click', () => toggleAutoRenew(sw.getAttribute('data-renew'), sw));
  });
}

async function loadBilling(){
  try {
    billingState = await getJSON('/api/billing');
    renderBilling();
  } catch (err) {
    document.getElementById('billingToggles').innerHTML =
      '<div class="rw-hint">Could not load billing details.</div>';
  }
}

async function toggleAutoRenew(product, sw){
  const msg = document.getElementById('billingMsg');
  const enabled = !sw.classList.contains('on');
  sw.classList.add('busy');
  try {
    await postJSON('/api/billing/auto-renew', { product, enabled });
    await loadBilling();
    flashMsg(msg, enabled ? 'Auto-renew is on.' : 'Auto-renew is off.', true);
  } catch (err) {
...
document.getElementById('billingRemoveCardBtn').addEventListener('click', async () => {
  const msg = document.getElementById('billingMsg');
  const btn = document.getElementById('billingRemoveCardBtn');
  btn.disabled = true;
  try {
    await postJSON('/api/billing/card/remove', {});
    await loadBilling();
    flashMsg(msg, 'Card removed. Auto-renew has been turned off.', true);
  } catch (err) {
    flashMsg(msg, err.message, false);
  } finally {
    btn.disabled = false;
...
let scEditingMessageId = null;
let scReplyingTo = null;
...
function renderSupportThreadRow(t){
  ...
  function attachPreviewIcon(type){
    if (!type || !['image', 'file', 'voice'].includes(type)) return '';
    return '<span class="sc-pv-ico sc-pv-' + type + '" aria-hidden="true"></span>';
  }
  info.innerHTML =
    '<div class="sc-thread-name">' + esc(name) + (t.verified ? VERIFIED_BADGE : '') + '</div>' +
    '<div class="sc-thread-preview">' + attachPreviewIcon(t.lastMessageType) + esc(t.lastMessageText || '') + '</div>';
...
async function openSupportInbox(){
  ...
  list.innerHTML = '<div class="sk-stack"><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div></div>';
  try {
...
function renderReplyQuoteHtml(replyTo){
  if (!replyTo) return '';
  const label = supportChatIsAdminView
    ? (replyTo.fromAdmin ? 'You' : 'Customer')
    : (replyTo.fromAdmin ? 'Customer Care' : 'You');
  return '<div class="sc-reply-quote" data-jump-to="' + esc(replyTo.id) + '">' +
    '<span class="sc-reply-quote-label">' + esc(label) + '</span>' +
    '<span class="sc-reply-quote-text">' + esc(replyTo.text || '') + '</span>' +
  '</div>';
}
...
function renderSupportBubble(m){
  ...
  return '<div class="sc-msg-row ' + (mine ? 'sc-row-me' : 'sc-row-them') + '" id="scmsg-' + esc(m.id) + '" data-msg-id="' + esc(m.id) + '" data-mine="' + (mine ? '1' : '0') + '" data-created-at="' + m.createdAt + '">' +
    ...
    '<div class="sc-msg-row-inner">' + bubble + '</div>' +
  '</div>';
}
...
let scMsgMenuTargetId = null;

function openMsgMenuFor(msgId){
  const msg = currentChatMessagesById[msgId];
  if (!msg) return;
  const mine = supportChatIsAdminView ? msg.fromAdmin : !msg.fromAdmin;
  if (!mine) return;
  scMsgMenuTargetId = msgId;
  const age = Date.now() - msg.createdAt;
  document.getElementById('scMsgEditBtn').disabled = !msg.text || age > 15 * 60 * 1000;
  document.getElementById('scMsgDeleteBtn').disabled = age > 24 * 60 * 60 * 1000;
  document.getElementById('scMsgMenuOverlay').classList.add('show');
}

function wireMessageRowGestures(container){
  container.querySelectorAll('.sc-msg-row').forEach((row) => {
    const msgId = row.dataset.msgId;
    const inner = row.querySelector('.sc-msg-row-inner');
    const replyIcon = row.querySelector('.sc-reply-icon');
    let startX = 0, startY = 0, moved = false, dragging = false, pressTimer = null;

    function startPress(x, y){
      startX = x; startY = y; moved = false; dragging = false;
      pressTimer = setTimeout(() => { if (!moved) openMsgMenuFor(msgId); }, 450);
    }
    ...
    function endPress(x){
      ...
      if (dragging) {
        ...
        if (Math.abs(dx) > 55) {
          const msg = currentChatMessagesById[msgId];
          if (msg) enterReplyMode(msg);
        }
      }
      dragging = false;
    }

    row.addEventListener('touchstart', (e) => startPress(e.touches[0].clientX, e.touches[0].clientY), { passive: true });
    row.addEventListener('touchmove', (e) => movePress(e.touches[0].clientX, e.touches[0].clientY), { passive: true });
    row.addEventListener('touchend', (e) => endPress(e.changedTouches[0].clientX));
    row.addEventListener('touchcancel', () => { clearTimeout(pressTimer); inner.style.transform = ''; replyIcon.classList.remove('show'); dragging = false; });
  });

  container.querySelectorAll('.sc-reply-quote').forEach((q) => {
    q.addEventListener('click', () => {
      const target = document.getElementById('scmsg-' + q.dataset.jumpTo);
      if (!target) return;
      target.scrollIntoView({ behavior: 'smooth', block: 'center' });
      target.classList.add('sc-jump-highlight');
      setTimeout(() => target.classList.remove('sc-jump-highlight'), 900);
    });
  });
}
...
async function openSupportChat(uid, title, isAdminView, verified, lastActiveAt, photoURL){
  ...
  document.getElementById('supportChatInput').style.height = 'auto';
...
function clearComposerContext(){
  scEditingMessageId = null;
  scReplyingTo = null;
  document.getElementById('scComposerContext').style.display = 'none';
}

function enterReplyMode(msg){
  scEditingMessageId = null;
  scReplyingTo = { id: msg.id, text: msg.text || (msg.attachmentType ? '[attachment]' : ''), fromAdmin: !!msg.fromAdmin };
  const mineLabel = supportChatIsAdminView ? 'You' : 'Customer Care';
  const themLabel = supportChatIsAdminView ? 'Customer' : 'You';
  document.getElementById('scComposerContextLabel').textContent = 'Replying to ' + (msg.fromAdmin ? mineLabel : themLabel);
  document.getElementById('scComposerContextText').textContent = msg.text || (msg.attachmentType ? 'Attachment' : '');
  document.getElementById('scComposerContext').style.display = 'flex';
  document.getElementById('supportChatInput').focus();
}

function enterEditMode(msg){
  scReplyingTo = null;
  scEditingMessageId = msg.id;
  document.getElementById('scComposerContextLabel').textContent = 'Editing message';
  document.getElementById('scComposerContextText').textContent = msg.text || '';
  document.getElementById('scComposerContext').style.display = 'flex';
  const input = document.getElementById('supportChatInput');
...
async function sendSupportChatMessage(){
  ...
  if (scEditingMessageId) {
    if (!text) return;
    const editingId = scEditingMessageId;
    ...
    try {
      const url = supportChatIsAdminView
        ? '/api/admin/support/threads/' + encodeURIComponent(supportChatTargetUid) + '/messages/' + encodeURIComponent(editingId) + '/edit'
        : '/api/support/messages/' + encodeURIComponent(editingId) + '/edit';
  ...
  const replyPayload = scReplyingTo ? { id: scReplyingTo.id } : undefined;
  sendBtn.disabled = true;
  input.value = '';
  input.style.height = 'auto';
  clearPendingAttachment();
  ...
  try {
    ...
    if (replyPayload) body.replyTo = replyPayload;
...
function initSupportFab(){
  ...
  document.getElementById('scMsgEditBtn').addEventListener('click', () => {
    document.getElementById('scMsgMenuOverlay').classList.remove('show');
    const msg = currentChatMessagesById[scMsgMenuTargetId];
    if (msg) enterEditMode(msg);
  });
  document.getElementById('scMsgDeleteBtn').addEventListener('click', async () => {
    document.getElementById('scMsgMenuOverlay').classList.remove('show');
    const msgId = scMsgMenuTargetId;
    if (!msgId || !confirm('Delete this message? This cannot be undone.')) return;
    try {
      const url = supportChatIsAdminView
        ? '/api/admin/support/threads/' + encodeURIComponent(supportChatTargetUid) + '/messages/' + encodeURIComponent(msgId) + '/delete'
        : '/api/support/messages/' + encodeURIComponent(msgId) + '/delete';
      await postJSON(url, {});
      await loadSupportChatMessages(false);
    } catch (err) {
  ...
  document.getElementById('scMsgMenuCancelBtn').addEventListener('click', () => {
    document.getElementById('scMsgMenuOverlay').classList.remove('show');
  });
  document.getElementById('scMsgMenuOverlay').addEventListener('click', (e) => {
    if (e.target.id === 'scMsgMenuOverlay') document.getElementById('scMsgMenuOverlay').classList.remove('show');
  });
  document.getElementById('scComposerContextCancel').addEventListener('click', () => {
    const wasEditing = !!scEditingMessageId;
    clearComposerContext();
    if (wasEditing) {
      const input = document.getElementById('supportChatInput');
      input.value = '';
  ...
  chatInput.addEventListener('input', () => { updateSendBtnEnabled(); autoGrowChatInput(); });
...
if (localStorage.getItem(ACC_VIEW_STORAGE_KEY) === 'rewards' || window.location.hash === '#rewards') {
  showRewards();
  loadRewardsSummary();
}
...
async function loadRewardsSummary(){
  ...
  const sendMin = rewardsSummary.minCoinTransfer || 5;
  const sendMax = rewardsSummary.maxCoinTransfer || 100;
  const sendAmountEl = document.getElementById('rwSendAmount');
  sendAmountEl.min = String(sendMin);
  sendAmountEl.max = String(sendMax);
  sendAmountEl.placeholder = String(sendMin);
  document.getElementById('rwSendLimits').innerHTML =
    '<span>Minimum <b>' + sendMin + ' coins</b>, maximum <b>' + sendMax + ' coins</b> per transfer.</span>' +
    '<span>You have <b>' + (rewardsSummary.coinBalance || 0) + ' coins</b>.</span>';
  updateSendButton();
  updateReceiveRate();
...
const rwSendOverlay = document.getElementById('rwSendOverlay');
const rwReceiveOverlay = document.getElementById('rwReceiveOverlay');
const rwCardsOverlay = document.getElementById('rwCardsOverlay');
const rwClaimOverlay = document.getElementById('rwClaimOverlay');

function openRwOverlay(overlay){ overlay.classList.add('show'); }
function closeRwOverlay(overlay){ overlay.classList.remove('show'); }

function bindRwOverlay(overlay, closeBtnId){
  document.getElementById(closeBtnId).addEventListener('click', () => closeRwOverlay(overlay));
  overlay.addEventListener('click', (e) => { if (e.target === overlay) closeRwOverlay(overlay); });
}

bindRwOverlay(rwSendOverlay, 'rwSendOverlayCloseBtn');
bindRwOverlay(rwCardsOverlay, 'rwCardsCloseBtn');

document.getElementById('rwActionSend').addEventListener('click', () => {
  const msg = document.getElementById('rwSendMsg');
  msg.className = 'acc-msg';
  msg.textContent = '';
  openRwOverlay(rwSendOverlay);
});

document.getElementById('rwActionCards').addEventListener('click', () => {
  openRwOverlay(rwCardsOverlay);
});

document.getElementById('rwActionReceive').addEventListener('click', () => {
  resetReceiveOverlay();
  openRwOverlay(rwReceiveOverlay);
});

const receiveCoinsInput = document.getElementById('rwReceiveCoins');
const receiveRateEl = document.getElementById('rwReceiveRate');
const receiveGenBtn = document.getElementById('rwReceiveGenBtn');
const receiveFormEl = document.getElementById('rwReceiveForm');
const receiveResultEl = document.getElementById('rwReceiveResult');
const receiveLinkInput = document.getElementById('rwReceiveLink');
const receiveMsg = document.getElementById('rwReceiveMsg');

function receiveLimits(){
  return {
    rate: (rewardsSummary && rewardsSummary.coinRequestRateNgn) || 5,
    min: (rewardsSummary && rewardsSummary.minCoinRequest) || 20,
    max: (rewardsSummary && rewardsSummary.maxCoinRequest) || 5000,
    ttlMin: Math.round(((rewardsSummary && rewardsSummary.coinRequestTtlMs) || 3600000) / 60000),
  };
}

function updateReceiveRate(){
  const limits = receiveLimits();
  const coins = Math.floor(Number(receiveCoinsInput.value));
  if (!Number.isFinite(coins) || coins <= 0) {
    receiveRateEl.innerHTML = 'Rate: <b>' + fmtNgn(limits.rate) + ' per coin</b>. Ask for <b>' + limits.min + '</b> to <b>' + limits.max + ' coins</b>.';
    return;
  }
  if (coins < limits.min || coins > limits.max) {
    receiveRateEl.innerHTML = 'Ask for between <b>' + limits.min + '</b> and <b>' + limits.max + ' coins</b>.';
    return;
  }
  receiveRateEl.innerHTML = 'They pay <b>' + fmtNgn(coins * limits.rate) + '</b> for your <b>' + coins + ' coins</b>.';
}

function resetReceiveOverlay(){
  receiveLinkInput.value = '';
  document.getElementById('rwReceiveSummary').innerHTML = '';
  receiveResultEl.style.display = 'none';
  receiveFormEl.style.display = 'block';
  receiveMsg.className = 'acc-msg';
  receiveMsg.textContent = '';
  const limits = receiveLimits();
  receiveCoinsInput.min = String(limits.min);
  receiveCoinsInput.max = String(limits.max);
  receiveCoinsInput.placeholder = String(limits.min);
  updateReceiveRate();
}

receiveCoinsInput.addEventListener('input', updateReceiveRate);

document.getElementById('rwReceiveCloseBtn').addEventListener('click', () => {
  closeRwOverlay(rwReceiveOverlay);
  resetReceiveOverlay();
});
rwReceiveOverlay.addEventListener('click', (e) => {
  if (e.target !== rwReceiveOverlay) return;
  closeRwOverlay(rwReceiveOverlay);
  resetReceiveOverlay();
});

document.getElementById('rwReceiveAgainBtn').addEventListener('click', () => {
  receiveCoinsInput.value = '';
  resetReceiveOverlay();
});

document.getElementById('rwReceiveCopyBtn').addEventListener('click', () => {
  navigator.clipboard.writeText(receiveLinkInput.value).catch(() => {});
  const btn = document.getElementById('rwReceiveCopyBtn');
  const original = btn.textContent;
  btn.textContent = 'Copied!';
  setTimeout(() => { btn.textContent = original; }, 1400);
});

receiveGenBtn.addEventListener('click', async () => {
  const limits = receiveLimits();
  const coins = Math.floor(Number(receiveCoinsInput.value));
  if (!Number.isFinite(coins) || coins < limits.min || coins > limits.max) {
    flashMsg(receiveMsg, 'Enter an amount between ' + limits.min + ' and ' + limits.max + ' coins.', false);
    return;
  }
  const original = receiveGenBtn.innerHTML;
  receiveGenBtn.disabled = true;
  receiveGenBtn.innerHTML = '<span class="btn-spinner"></span>Generating…';
  try {
    const data = await postJSON('/api/rewards/payment-link', { coins });
    receiveLinkInput.value = data.url;
    document.getElementById('rwReceiveSummary').innerHTML =
      'Whoever opens it pays <b>' + fmtNgn(data.amountNgn) + '</b>, and <b>' + data.coins + ' coins</b> land in your balance. ' +
      'The link stops working after <b>' + limits.ttlMin + ' minutes</b> or once it is paid.';
    receiveFormEl.style.display = 'none';
    receiveResultEl.style.display = 'block';
    receiveMsg.className = 'acc-msg';
    receiveMsg.textContent = '';
  } catch (err) {
    flashMsg(receiveMsg, err.message, false);
  } finally {
    receiveGenBtn.disabled = false;
    receiveGenBtn.innerHTML = original;
  }
});

const claimContinueBtn = document.getElementById('rwClaimContinueBtn');
const claimMsg = document.getElementById('rwClaimMsg');
let claimCaptchaPassed = false;
let claimCaptchaValue = '';

document.getElementById('rwClaimAltcha').addEventListener('statechange', (ev) => {
  claimCaptchaPassed = ev.detail.state === 'verified';
  claimCaptchaValue = claimCaptchaPassed ? ev.detail.payload : '';
  claimContinueBtn.disabled = !claimCaptchaPassed;
});

bindRwOverlay(rwClaimOverlay, 'rwClaimCloseBtn');

const claimCheckingEl = document.getElementById('rwClaimChecking');
const claimReadyEl = document.getElementById('rwClaimReady');
const claimNoFaceIdEl = document.getElementById('rwClaimNoFaceId');
const claimTitleEl = document.getElementById('rwClaimTitle');
const claimSubEl = document.getElementById('rwClaimSub');

function setClaimState(state){
  claimCheckingEl.style.display = state === 'checking' ? 'block' : 'none';
  claimReadyEl.style.display = state === 'ready' ? 'block' : 'none';
  claimNoFaceIdEl.style.display = state === 'nofaceid' ? 'block' : 'none';
  if (state === 'nofaceid') {
    claimTitleEl.textContent = 'Face Id not Set';
    claimSubEl.textContent = 'Go to Face Id and configure';
  } else {
    claimTitleEl.textContent = 'Claim Daily Coins';
    claimSubEl.textContent = 'Verify the captcha, then hold still for a quick face check.';
  }
}

document.getElementById('rwClaimGoFaceIdBtn').addEventListener('click', () => {
...
document.getElementById('rwDailyClaimBtn').addEventListener('click', async () => {
  ...
  let enabled = false;
  try {
    const status = await getJSON('/api/facescan/status');
    enabled = !!status.enabled;
  } catch (err) {
    enabled = true;
  }
  setClaimState(enabled ? 'ready' : 'nofaceid');
...
claimContinueBtn.addEventListener('click', async () => {
  ...
  let capture = null;
  try {
    claimModule = await import('/claim-face.js');
    capture = await claimModule.captureClaimFace();
  } catch (err) {
    claimContinueBtn.innerHTML = original;
    claimContinueBtn.disabled = !claimCaptchaPassed;
    flashMsg(claimMsg, (err && err.message) || 'Face check failed. Please try again.', false);
    return;
  }

  claimContinueBtn.innerHTML = '<span class="btn-spinner"></span>Claiming…';
  try {
    const faceDescriptor = (capture.samples && capture.samples.length)
      ? capture.samples.map((v) => ({ v }))
      : capture.descriptor;
    ...
    flashMsg(document.getElementById('rwDailyClaimMsg'), '+' + result.amount + ' coins claimed!', true);
...
const BANK_BRANDS = {
  'Access Bank': { mono: 'A', label: 'Access', color: '#F26522', motif: 'access' },
  'Carbon': { mono: 'C', label: 'Carbon', color: '#101820' },
  'Citibank Nigeria': { mono: 'C', label: 'Citibank', color: '#003882' },
  'Ecobank Nigeria': { mono: 'E', label: 'Ecobank', color: '#00335E' },
  'FairMoney MFB': { mono: 'F', label: 'FairMoney', color: '#6C2EB9' },
  'Fidelity Bank': { mono: 'F', label: 'Fidelity', color: '#00447C' },
  'First Bank of Nigeria': { mono: 'FBN', label: 'First Bank', color: '#00447B' },
  'First City Monument Bank (FCMB)': { mono: 'FC', label: 'FCMB', color: '#00338D' },
  'Globus Bank': { mono: 'G', label: 'Globus', color: '#00A99D' },
  'Guaranty Trust Bank (GTBank)': { mono: 'GT', label: 'GTBank', color: '#EE6C0F' },
  'Jaiz Bank': { mono: 'J', label: 'Jaiz', color: '#00693E' },
  'Keystone Bank': { mono: 'K', label: 'Keystone', color: '#EE7623' },
  'Kuda Bank': { mono: 'k', label: 'Kuda', color: '#3D1E6D' },
  'Moniepoint MFB': { mono: 'M', label: 'Moniepoint', color: '#052D5D' },
  'OPay': { mono: 'O', label: 'OPay', color: '#00A860' },
  'Optimus Bank': { mono: 'OB', label: 'Optimus', color: '#0B6E5C' },
  'PalmPay': { mono: 'P', label: 'PalmPay', color: '#8A2EE0' },
  'Parallex Bank': { mono: 'PX', label: 'Parallex', color: '#1B4F9C' },
  'Polaris Bank': { mono: 'PB', label: 'Polaris', color: '#ED1C24' },
  'Premium Trust Bank': { mono: 'PT', label: 'Premium Trust', color: '#1C8A4B' },
  'Providus Bank': { mono: 'PV', label: 'Providus', color: '#1B2A4A' },
  'Rubies MFB': { mono: 'R', label: 'Rubies', color: '#E4287C' },
  'Signature Bank': { mono: 'S', label: 'Signature', color: '#1A1A1A' },
  'Sparkle Microfinance Bank': { mono: 'S', label: 'Sparkle', color: '#F7A600' },
  'Stanbic IBTC Bank': { mono: 'SI', label: 'Stanbic IBTC', color: '#0033A1' },
  'Standard Chartered Bank': { mono: 'SC', label: 'Std Chartered', color: '#0473EA' },
  'Sterling Bank': { mono: 'S', label: 'Sterling', color: '#A6192E' },
  'SunTrust Bank': { mono: 'SU', label: 'SunTrust', color: '#F37335' },
  'Titan Trust Bank': { mono: 'TT', label: 'Titan Trust', color: '#12294B' },
  'Union Bank of Nigeria': { mono: 'UB', label: 'Union Bank', color: '#F58220' },
  'United Bank for Africa (UBA)': { mono: 'U', label: 'UBA', color: '#D71921' },
  'Unity Bank': { mono: 'UN', label: 'Unity Bank', color: '#00A651' },
  'VFD Microfinance Bank': { mono: 'VF', label: 'VFD', color: '#0F3D3E' },
  'Wema Bank': { mono: 'W', label: 'Wema Bank', color: '#6E2585' },
  'Zenith Bank': { mono: 'Z', label: 'Zenith', color: '#E4002B', motif: 'zenith' },
};
const DEFAULT_BANK_BRAND = { mono: 'BK', label: '', color: '#5a4fcf' };

function bankMotifHtml(motif){
  switch (motif) {
    case 'zenith':
      return '<span class="bm-zenith"></span>';
    case 'access':
      return '<span class="bm-access-r2"></span><span class="bm-access-r1"></span><span class="bm-access-core"></span>';
    default:
      return '';
  }
}
...
function renderBankCard(){
  ...
  if (bd) {
    ...
    const logoClass = 'rw-bank-card-logo' + (brand.mono.length > 1 ? ' wide' : '');
    const FULL_REPLACE_MOTIFS = ['zenith', 'access'];
    const showMono = !brand.motif || !FULL_REPLACE_MOTIFS.includes(brand.motif);
    const monoHtml = showMono ? '<span class="rw-bank-card-logo-mono">' + esc(brand.mono) + '</span>' : '';
    document.getElementById('rwBankCardBrand').innerHTML =
      '<span class="' + logoClass + '" style="--bm-fg:' + (brand.color || '#5a4fcf') + '">' +
      monoHtml + bankMotifHtml(brand.motif) +
      '</span>' +
      '<span class="rw-bank-card-brand-text">' + esc(brand.label || bd.bankName) + '</span>';
...
let sendCaptchaPassed = false;
let sendCaptchaValue = '';
let sendUsernameOk = false;
let sendUsernameSeq = 0;

const sendUsernameInput = document.getElementById('rwSendUsername');
const sendAmountInput = document.getElementById('rwSendAmount');
const sendBtn = document.getElementById('rwSendBtn');
const sendMsg = document.getElementById('rwSendMsg');
const sendStatus = document.getElementById('rwSendUsernameStatus');

function sendLimits(){
  return {
    min: (rewardsSummary && rewardsSummary.minCoinTransfer) || 5,
    max: (rewardsSummary && rewardsSummary.maxCoinTransfer) || 100,
  };
}

function updateSendButton(){
  const { min, max } = sendLimits();
  const amount = Math.floor(Number(sendAmountInput.value));
  const balance = (rewardsSummary && rewardsSummary.coinBalance) || 0;
  const amountOk = Number.isFinite(amount) && amount >= min && amount <= max && amount <= balance;
  sendBtn.disabled = !(sendUsernameOk && amountOk);
}

sendUsernameInput.addEventListener('input', () => {
  const val = sendUsernameInput.value.trim().toLowerCase();
  const seq = ++sendUsernameSeq;
  sendUsernameOk = false;
  updateSendButton();

  if (!val) { sendStatus.className = 'uname-status'; sendStatus.innerHTML = ''; return; }
  if (!/^[a-z0-9_]{3,20}$/.test(val)) {
    sendStatus.className = 'uname-status taken';
    sendStatus.textContent = '3-20 characters: letters, numbers, underscore only.';
    return;
  }
  sendStatus.className = 'uname-status';
  sendStatus.innerHTML = '<span class="uname-spinner"></span>Checking Username ..';
  setTimeout(async () => {
    if (seq !== sendUsernameSeq) return;
    try {
      const res = await fetch('/api/rewards/lookup-user?username=' + encodeURIComponent(val));
      const data = await res.json();
      if (seq !== sendUsernameSeq) return;
      if (data.exists) {
        sendUsernameOk = true;
        sendStatus.className = 'uname-status ok';
        sendStatus.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M20 6L9 17l-5-5"/></svg>Username exists';
      } else {
        sendUsernameOk = false;
        sendStatus.className = 'uname-status taken';
        sendStatus.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" d="M6 6l12 12M18 6L6 18"/></svg>' + esc(data.error || "Username doesn't exist");
      }
    } catch {
      if (seq !== sendUsernameSeq) return;
      sendUsernameOk = false;
      sendStatus.className = 'uname-status taken';
      sendStatus.textContent = 'Could not check username right now, try again in a moment.';
    }
    updateSendButton();
  }, 450);
});

sendAmountInput.addEventListener('input', updateSendButton);

const sendConfirmOverlay = document.getElementById('rwSendConfirmOverlay');
const sendConfirmBtn = document.getElementById('rwSendConfirmBtn');
const sendConfirmMsg = document.getElementById('rwSendConfirmMsg');

document.getElementById('rwSendAltcha').addEventListener('statechange', (ev) => {
  sendCaptchaPassed = ev.detail.state === 'verified';
  sendCaptchaValue = sendCaptchaPassed ? ev.detail.payload : '';
  sendConfirmBtn.disabled = !sendCaptchaPassed;
});

function closeSendConfirm(){
  sendConfirmOverlay.classList.remove('show');
  openRwOverlay(rwSendOverlay);
}

function finishSendFlow(){
  sendConfirmOverlay.classList.remove('show');
  closeRwOverlay(rwSendOverlay);
}

sendBtn.addEventListener('click', () => {
  const { min, max } = sendLimits();
  const username = sendUsernameInput.value.trim().toLowerCase();
  const amount = Math.floor(Number(sendAmountInput.value));
  const balance = (rewardsSummary && rewardsSummary.coinBalance) || 0;

  if (!sendUsernameOk) { flashMsg(sendMsg, 'Enter a username that exists.', false); return; }
  if (!Number.isFinite(amount) || amount < min || amount > max) {
    flashMsg(sendMsg, 'Enter an amount between ' + min + ' and ' + max + ' coins.', false);
    return;
  }
  if (amount > balance) {
    flashMsg(sendMsg, 'You only have ' + balance + ' coins.', false);
    return;
  }

  document.getElementById('rwSendConfirmSub').innerHTML =
    'Transfer <b>' + amount + ' coins</b> to <b>@' + esc(username) + '</b>?';
  sendConfirmMsg.className = 'acc-msg';
  sendConfirmMsg.textContent = '';
  sendConfirmBtn.disabled = !sendCaptchaPassed;
  closeRwOverlay(rwSendOverlay);
  sendConfirmOverlay.classList.add('show');
});

document.getElementById('rwSendCancelBtn').addEventListener('click', closeSendConfirm);
sendConfirmOverlay.addEventListener('click', (e) => { if (e.target === sendConfirmOverlay) closeSendConfirm(); });

sendConfirmBtn.addEventListener('click', async () => {
  const username = sendUsernameInput.value.trim().toLowerCase();
  const amount = Math.floor(Number(sendAmountInput.value));
  if (!sendCaptchaPassed) { flashMsg(sendConfirmMsg, 'Complete the captcha first.', false); return; }

  const originalHtml = sendConfirmBtn.innerHTML;
  sendConfirmBtn.disabled = true;
  sendConfirmBtn.innerHTML = '<span class="btn-spinner"></span>Sending…';
  try {
    const data = await postJSON('/api/rewards/transfer', { username, amount, altcha: sendCaptchaValue });
    finishSendFlow();
    showToast('Sent ' + data.amount + ' coins to @' + data.toUsername + '.');
    ...
    flashMsg(sendConfirmMsg, err.message, false);
  } finally {
    sendConfirmBtn.innerHTML = originalHtml;
    sendConfirmBtn.disabled = !sendCaptchaPassed;
    updateSendButton();
  }
});
...
export function renderAccountGuestGate(cfg) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
:root{
  ...
  --dark:#0A0A0F;--card:#15151F;
  --border:rgba(255,255,255,.1);
  --text:#F3F3FA;--muted:rgba(255,255,255,.5);
  --font-display:'Space Grotesk',Inter,-apple-system,sans-serif;
  --font-body:'Inter',-apple-system,sans-serif;
  --ease:cubic-bezier(.22,1,.36,1);
}
:root[data-theme="light"]{
  --dark:#F5F6FA;--card:#FFFFFF;--border:rgba(0,0,0,.1);
...
.aurora{position:fixed;inset:0;overflow:hidden;z-index:0;pointer-events:none}
.blob{position:absolute;border-radius:50%;filter:blur(65px);mix-blend-mode:screen}
.blob-1{width:500px;height:500px;background:radial-gradient(circle,var(--accent),transparent 70%);opacity:.4;top:-140px;left:-120px}
.blob-2{width:460px;height:460px;background:radial-gradient(circle,var(--accent2),transparent 70%);opacity:.35;bottom:-160px;right:-100px}
.gate-card{
...
.gate-icon svg{width:28px;height:28px;color:#04141a}
.gate-card h1{font-family:var(--font-display);font-size:1.3rem;margin-bottom:8px}
.gate-card p{color:var(--muted);font-size:.88rem;line-height:1.55;margin-bottom:22px}
.gate-btn{
  width:100%;background:linear-gradient(90deg,var(--accent),var(--accent2));border:none;color:#04141a;
  font-weight:700;font-size:.92rem;padding:13px;border-radius:12px;cursor:pointer;
  transition:transform .15s var(--ease),box-shadow .15s var(--ease);
}
.gate-btn:hover{transform:translateY(-1px);box-shadow:0 8px 20px rgba(0,224,255,.25)}
.gate-back{display:inline-block;margin-top:16px;color:var(--muted);text-decoration:none;font-size:.8rem}
.gate-back:hover{color:var(--accent)}
</style>
</head>
<body>
<div class="aurora"><div class="blob blob-1"></div><div class="blob blob-2"></div></div>
<div class="gate-card">
  <div class="gate-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4.4 3.6-8 8-8s8 3.6 8 8"/></svg></div>
  <h1>Sign in to your account</h1>
  <p>Your account page holds your own settings and data, so you will need to sign in first. Sign in now and you will land right back here.</p>
  <button class="gate-btn" id="gateSignInBtn">Sign In</button>
  <a class="gate-back" href="/">Back to ES TEAMS TV</a>
</div>
<script nonce="__CSP_NONCE__">
document.getElementById('gateSignInBtn').addEventListener('click', function(){
