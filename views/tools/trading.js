...
(function(){
  ...
  async function pollPositions(){
    ...
    try {
      ...
      if (data.equity == null && data.balanceError) {
        equityEl.title = data.balanceError;
        availEl.title = data.balanceError;
        if (!balanceErrorShown) { balanceErrorShown = true; toast(data.balanceError); }
      } else {
        equityEl.removeAttribute('title');
    ...
    try {
      ...
      list.innerHTML = trades.map(function(t, i){
        ...
        return '<div class="tr-list-item" style="cursor:pointer" data-trade-index="' + i + '">' +
          ...
          '<div class="tr-list-meta"><span>' + esc(t.side === 'Buy' ? 'Long' : 'Short') + ' \\u00b7 ' + (t.leverage != null ? t.leverage + 'x' : 'N/A') + '</span><span>Qty <b>' + esc(t.qty) + '</b></span></div>' +
