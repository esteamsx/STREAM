...
.db-link-card:hover{border-color:rgba(0,224,255,.42);transform:translateY(-1px)}
.db-link-card .db-card-sub{margin-bottom:0}
.db-link-body{flex:1;min-width:0}
.db-link-go{
  width:34px;height:34px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;
  background:rgba(0,224,255,.12);border:1px solid rgba(0,224,255,.3);color:var(--accent);
}
.db-link-go svg{width:16px;height:16px}
...
.db-btn-lock{background:var(--card2);color:var(--muted);border:1px solid var(--border-strong)}
.db-btn-lock:hover{filter:none;color:var(--text)}
.db-btn-lock svg{width:18px;height:18px}
...
<div class="db-wrap">
  ...
  <div class="db-card">
    ...
    <div id="botsList"><div class="sk-stack"><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div><div class="sk-row"><div class="sk-avatar"></div><div class="sk-row-body"><div class="sk-line w60"></div><div class="sk-line w30"></div></div></div></div></div>
  </div>

  <a class="db-card db-link-card" href="/channel-react">
    <div class="db-link-body">
      <div class="db-card-title">Channel Reaction</div>
      <div class="db-card-sub">Paste a WhatsApp channel post link and your connected bot runs the reaction for you.</div>
    </div>
    <span class="db-link-go">
...
<script nonce="__CSP_NONCE__">
  ...
  const LOCK_ICON = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="11" width="14" height="10" rx="2"/><path stroke-linecap="round" d="M8 11V7a4 4 0 018 0v4"/></svg>';
  ...
  async function refreshList(){
    ...
    if (list.querySelector('.db-empty, .sk-stack')) {
      list.innerHTML = '';
      cardsById.clear();
    }
  ...
  function updateDeployBtnLook(){
    ...
    if (!isVerified) {
      btn.classList.add('db-btn-lock');
      btn.classList.remove('db-btn-dead');
      btn.innerHTML = LOCK_ICON;
      return;
    }
    btn.classList.remove('db-btn-lock');
    btn.textContent = 'Deploy';
    btn.classList.toggle('db-btn-dead', atUserCap());
  }

  async function loadVerification(){
    try {
      const profile = await getJSON('/api/profile');
      isVerified = !!(profile && (profile.isAdmin || profile.verified));
    } catch {
      isVerified = false;
    }
    updateDeployBtnLook();
